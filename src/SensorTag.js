...
class SensorTag extends Component<Props, State> {
  ...
  async asa(): Promise<any> {
    const {writeWithResponse} = new Characteristic();
    // const res = await this.props.sensorTag.localName();
    console.log(
      'res',
      await this.props.sensorTag.discoverAllServicesAndCharacteristics(),
    );
  }
